...
import { Plus, Download, Upload, MapPin, Activity, Tag } from 'lucide-react';
...
import { countThongTinToChucQuanTrongColumnSearchActive } from '../utils/column-search';
...
interface Props {
  ...
  items?: ThongTinToChucQuanTrong[] | null;
}

const ThongTinToChucQuanTrongToolbar: React.FC<Props> = ({
  onPageBack,
  onAdd,
  onExport,
  onImport,
  ...
  const activeFilterCount = useMemo(() => {
    return (
      (searchTerm ? 1 : 0) +
      countThongTinToChucQuanTrongColumnSearchActive(filters.columnSearch ?? {}) +
  ...
  const handleClearAllFilters = () => {
    setSearchTerm('');
    useThongTinToChucQuanTrongStore.getState().setFilter('columnSearch', {});
    ...
    () => (
      <div className="flex flex-wrap items-center gap-2 min-w-0">
        <FilterChipMultiSelect
          ...
          onChange={(val) => setFilter('loai_hinh_filter', val)}
          placeholder={txt('danTocToChucQuanTrong.store.loaiHinhCol')}
          icon={Tag}
        ...
        <FilterChipMultiSelect
          ...
          placeholder={txt('danTocToChucQuanTrong.store.trangThaiCol')}
          icon={Activity}
          className="shrink-0 w-full min-w-0 sm:w-[min(180px,24vw)] sm:max-w-[220px]"
        />
        <FilterChipMultiSelect
          ...
          placeholder={txt('danTocToChucQuanTrong.store.donViCol')}
          icon={MapPin}
          className="shrink-0 w-full min-w-0 sm:w-[min(220px,28vw)] sm:max-w-[280px]"
        />
      </div>
    ),
    [loaiHinhOptions, trangThaiOptions, donViOptions, filters, setFilter],
  );
    ...
    () => [
      {
        ...
        options: loaiHinhOptions,
        value: filters.loai_hinh_filter,
        onChange: (val: string[]) => setFilter('loai_hinh_filter', val),
      },
      {
  ...
  return (
    <GenericToolbar
      ...
      searchPlaceholder={txt('danTocToChucQuanTrong.searchPlaceholder')}
...
export default ThongTinToChucQuanTrongToolbar;

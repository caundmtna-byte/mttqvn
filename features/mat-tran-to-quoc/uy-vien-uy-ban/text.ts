export const matTranUyVienUyBan = {
  noViewPermission: 'Bạn không có quyền xem danh sách ủy viên ủy ban.',
  noEmployeeBanner: 'Tài khoản chưa gắn hồ sơ nhân viên — không thể ghi nhận người tạo bản ghi.',
  tinhCap: 'MTTQ tỉnh',
  service: {
    noEmployeeProfile: 'Không xác định được nhân viên đăng nhập để ghi người tạo.',
    notFound: 'Không tìm thấy bản ghi ủy viên.',
  },
  deleteTitle: 'Xóa ủy viên?',
  deleteMessage: 'Bản ghi sẽ bị xóa vĩnh viễn.',
  bulkDeleteTitle: 'Xóa nhiều ủy viên?',
  bulkDeleteMessage: 'Sẽ xóa {{count}} bản ghi.',
  noExportData: 'Không có dữ liệu để xuất.',
  exportFileName: 'danh-sach-uy-vien-uy-ban',
  toast: {
    create: 'Đã thêm ủy viên.',
    update: 'Đã cập nhật ủy viên.',
    delete: 'Đã xóa {{count}} bản ghi.',
    importSuccess: 'Đã nhập {{count}} ủy viên.',
  },
  import: {
    rowError: 'Dòng {{row}}: {{message}}',
    templateName: 'mau-nhap-uy-vien-uy-ban',
    badNhiemKy: 'Không tìm thấy nhiệm kỳ (ten_nhiem_ky / nhiem_ky_id).',
    badCanBo: 'Không map được cán bộ (can_bo_id hoặc họ tên + ngày sinh khớp mttq_can_bo).',
  },
  validation: {
    nhiemKyRequired: 'Chọn nhiệm kỳ.',
    hoVaTenRequired: 'Nhập họ và tên.',
    canBoRequired: 'Chọn cán bộ từ Danh sách cán bộ.',
    canBoDuplicateNhiemKy: 'Cán bộ này đã là ủy viên trong nhiệm kỳ đã chọn.',
    maUvDuplicateNhiemKy: 'Mã UV này đã được dùng trong nhiệm kỳ đã chọn.',
    importCanBoRequired: 'Cần can_bo_id (số) hoặc cặp họ tên + ngày sinh khớp một cán bộ.',
    trangThamGiaInvalid: 'Chọn trạng thái tham gia: Đang tham gia hoặc Thôi tham gia.',
  },
  store: {
    canBoIdCol: 'ID cán bộ (can_bo_id)',
    importHoTenMapCol: 'Họ và tên (map cán bộ khi không có can_bo_id)',
    hoVaTenCol: 'Họ và tên',
    maUvCol: 'Mã UV',
    tenNhiemKyCol: 'Nhiệm kỳ',
    donViCol: 'Đơn vị',
    chucVuDonViCol: 'Chức vụ đơn vị',
    trangThamGiaCol: 'Trạng thái tham gia',
    soKyHopCol: 'Số kỳ họp',
    diemDanhCoMatCol: 'Có mặt',
    diemDanhVangMatCol: 'Vắng mặt',
    diemDanhChuaCol: 'Chưa điểm danh',
    nguoiTaoCol: 'Người tạo',
    tgCapNhatCol: 'Cập nhật',
  },
  toolbar: {
    title: 'Ủy viên ủy ban',
  },
  trangThamGiaChuaGan: 'Chưa gán trạng thái',
  emptyTitle: 'Chưa có ủy viên',
  emptyHint: 'Thêm ủy viên mới để bắt đầu.',
  emptyFilteredHint: 'Thử xóa bộ lọc hoặc đổi từ khóa tìm kiếm.',
  form: {
    canBo: 'Cán bộ',
    sectionCanBo: 'Hồ sơ cán bộ (Danh sách cán bộ)',
    snapshotHint:
      'Các trường dưới đây chỉ xem — bạn không có quyền sửa hồ sơ cán bộ hoặc chưa chọn cán bộ.',
    snapshotEditableHint:
      'Chỉnh sửa trực tiếp tại đây. Khi bấm Lưu, hệ thống cập nhật bảng Danh sách cán bộ trước, sau đó lưu thông tin ủy viên.',
    canBoProfileLoading: 'Đang tải hồ sơ cán bộ…',
    diaChiCanBo: 'Địa chỉ (hồ sơ cán bộ)',
    sectionMain: 'Thông tin chính',
    sectionCaNhan: 'Nhân thân & tham gia',
    sectionHocVan: 'Trình độ & dân tộc - tôn giáo',
    sectionDang: 'Đảng viên',
    sectionLienHe: 'Liên hệ & địa chỉ',
    sectionGhiChu: 'Ghi chú',
    maUv: 'Mã UV',
    nhiemKy: 'Nhiệm kỳ',
    donVi: 'Đơn vị (xã/phường)',
    donViPlaceholder: 'Để trống = MTTQ tỉnh',
    hoVaTen: 'Họ và tên',
    chucVuDonVi: 'Chức vụ đơn vị',
    ngaySinh: 'Ngày sinh',
    gioiTinh: 'Giới tính',
    trangThamGia: 'Trạng thái tham gia',
    ngayNhapTrangThai: 'Ngày nhập trạng thái',
    vanHoa: 'Văn hóa',
    trinhDoCm: 'Trình độ CM',
    trinhDoLlct: 'Trình độ LLCT',
    danToc: 'Dân tộc',
    tonGiao: 'Tôn giáo',
    dangVien: 'Đảng viên',
    ngayVaoDang: 'Ngày vào Đảng',
    queQuan: 'Quê quán',
    noiOHienNay: 'Nơi ở hiện nay',
    soDienThoai: 'Số điện thoại',
    ghiChu: 'Ghi chú',
    createSubtitle: 'Thêm ủy viên mới',
    editSubtitle: 'Sửa ủy viên',
  },
  detail: {
    title: 'Chi tiết ủy viên ủy ban',
    tabInfo: 'Thông tin',
    tabDiemDanh: 'Điểm danh',
    diemDanhSectionTitle: 'Điểm danh theo kỳ họp',
    diemDanhEmptyKyHop: 'Chưa có kỳ họp trong nhiệm kỳ này.',
    sectionMain: 'Thông tin chính',
    sectionCaNhan: 'Nhân thân & tham gia',
    sectionHocVan: 'Trình độ & dân tộc - tôn giáo',
    sectionDang: 'Đảng viên',
    sectionLienHe: 'Liên hệ & địa chỉ',
    systemInfo: 'Thông tin hệ thống',
    tgTao: 'Thời điểm tạo',
    tgCapNhat: 'Cập nhật lần cuối',
  },
  printPreview: {
    pageTitle: 'Xem trước — Danh sách ủy viên theo nhiệm kỳ',
    print: 'In danh sách ủy viên',
    printPopupBlocked: 'Trình duyệt chặn cửa sổ in. Cho phép popup cho trang này.',
    chonNhiemKy: 'Chọn đúng một nhiệm kỳ ở bộ lọc trước khi in danh sách ủy viên.',
    notFound: 'Không tìm thấy nhiệm kỳ.',
    fileName: 'danh-sach-uy-vien-nhiem-ky',
    empty: 'Không có ủy viên trong phạm vi xem.',
    coQuanChuQuan: 'Ủy ban Mặt trận Tổ quốc Việt Nam tỉnh Nghệ An',
    banThuongTruc: 'Ban Thường trực',
    coQuanMacDinh: 'Ban Thường trực Ủy ban MTTQ Việt Nam tỉnh Nghệ An',
    soKyHieuTrong: '……../DS-MTTQ',
    tenLoai: 'Danh sách ủy viên Ủy ban MTTQ Việt Nam',
    trichYeu: 'Nhiệm kỳ {{nhiemKy}}',
    metaNhiemKy: 'Nhiệm kỳ',
    metaTongSo: 'Tổng số ủy viên',
    metaNu: 'Trong đó nữ',
    metaDanTocThieuSo: 'Dân tộc thiểu số',
    metaDangVien: 'Đảng viên',
    metaNgayIn: 'Ngày in',
    colStt: 'STT',
    colHoTen: 'Họ và tên',
    colNgaySinh: 'Ngày sinh',
    colGioiTinh: 'Giới tính',
    colDanToc: 'Dân tộc',
    colChucVuDonVi: 'Chức vụ, đơn vị công tác',
    colTrangThai: 'Trạng thái tham gia',
    kyThayMat: 'TM. Ban Thường trực',
    kyChucDanh: 'Chủ tịch',
    kyNguoiLap: 'Người lập biểu',
  },
} as const;

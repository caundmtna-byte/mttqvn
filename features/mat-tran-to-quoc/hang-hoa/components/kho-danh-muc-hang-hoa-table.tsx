...
import { KhoDanhMucHangHoaTableRowActions } from './kho-danh-muc-hang-hoa-table-row-actions';
...
const KhoDanhMucHangHoaTable = memo(function KhoDanhMucHangHoaTable({
  ...
  const renderCell = useCallback(
    (colId: string, item: KhoDanhMucHangHoaListRow) => {
      switch (colId) {
        case 'thu_tu':
        ...
        case 'ten_danh_muc':
          return (
            <div className="flex min-w-0 items-center gap-2">
              <FolderOpen size={14} className="shrink-0 text-primary/70" aria-hidden />
              <span className="truncate font-semibold text-foreground text-sm">{item.ten_danh_muc}</span>
            </div>
          );
        case 'mo_ta':
          return (
            <span className="text-body-sm text-muted-foreground truncate max-w-[min(320px,50vw)]" title={item.mo_ta ?? undefined}>
        ...
        case 'actions':
          return (
            <KhoDanhMucHangHoaTableRowActions
              item={item}
  ...
  const renderMobileCard = useCallback(
    (item: KhoDanhMucHangHoaListRow, isSelected: boolean) => (
      <div
        key={item.id}
        role="button"
        tabIndex={0}
        ...
        <div className="flex items-start gap-4">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex justify-between items-start mb-1">
              <h4 className="font-semibold text-foreground truncate">{item.ten_danh_muc}</h4>
              <input
                type="checkbox"
                checked={isSelected}
            ...
            <p className="text-xs text-muted-foreground m-0 flex items-center gap-1">
              ...
              {txt('matTranHangHoa.store.thuTu')}: {item.thu_tu}
            </p>
...
export default KhoDanhMucHangHoaTable;

...
const MttqThietLapDetail: React.FC<Props> = ({ data, onClose, onEdit, onDelete }) => {
  const { canEdit, canDelete } = useResourcePermissions('matTranThietLapCaiDat');
  const loaiLabel = txt(MTTQ_LOAI_TAB_LABEL_KEY[data.loai]);
  ...
  return (
    <GenericDrawer
      title={txt('page.matTranThietLap.detailTitle')}
      ...
      <div className="space-y-5">
        <div className="bg-card p-4 rounded-xl border border-border/50 shadow-sm flex items-center gap-4">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-body-sm text-muted-foreground mt-0.5">{loaiLabel}</p>
          </div>
        </div>
...
export default MttqThietLapDetail;

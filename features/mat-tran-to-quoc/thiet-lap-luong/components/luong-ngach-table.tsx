...
import { LuongNgachTableRowActions } from './luong-ngach-table-row-actions';
...
const LuongNgachTable = memo(function LuongNgachTable({
  ...
  const renderCell = useCallback(
    (colId: string, item: LuongThietLapNgachListRow) => {
      switch (colId) {
        case 'thu_tu':
          return (
        ...
        case 'ma':
          return (
            <span className="text-body-sm text-muted-foreground truncate" title={item.ma ?? undefined}>
              {item.ma ?? txt('common.emptyCell')}
            </span>
          );
        case 'ten':
          return (
            <div className="flex min-w-0 items-center gap-2">
              <Layers size={14} className="shrink-0 text-primary/70" aria-hidden />
  ...
  const renderMobileCard = useCallback(
    (item: LuongThietLapNgachListRow, isSelected: boolean) => (
      <div
        key={item.id}
        role="button"
        tabIndex={0}
      ...
      >
        <div className="flex items-start gap-4">
          ...
          <div className="flex-1 min-w-0">
            ...
            {item.ma ? <p className="text-xs text-muted-foreground m-0">{item.ma}</p> : null}
...
export default LuongNgachTable;

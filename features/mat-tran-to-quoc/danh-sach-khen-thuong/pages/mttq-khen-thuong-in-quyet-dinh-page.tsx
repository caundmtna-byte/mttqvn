...
import { buildKhenThuongQuyetDinhDocumentModel } from '../utils/build-khen-thuong-quyet-dinh-document';
...
export const KHEN_THUONG_LIST_PATH =
  '/mat-tran-to-quoc/tap-huan-khen-thuong/danh-sach-khen-thuong';
const PREVIEW_PREFIX = 'mttq-van-ban-preview';
const PRINT_ROOT_ID = 'khen-thuong-quyet-dinh-print-root';

const MttqKhenThuongInQuyetDinhPage: React.FC = () => {
  ...
  const docModel = useMemo(
    () =>
      kt ? buildKhenThuongQuyetDinhDocumentModel(kt, { company, viewer, canBoMap }) : null,
    [kt, company, viewer, canBoMap],
  );

  const fileBase = useMemo(() => {
    const base = txt('matTranKhenThuong.printPreview.fileNameQuyetDinh');
    const so = docModel?.soKyHieuGon?.trim();
    return so ? `${base}-${so}` : base;
  }, [docModel?.soKyHieuGon]);
  ...
  const handlePrint = useCallback(() => {
    ...
    const ok = printVanBanDocument(el, txt('matTranKhenThuong.printPreview.pageTitleQuyetDinh'));
...
export default MttqKhenThuongInQuyetDinhPage;

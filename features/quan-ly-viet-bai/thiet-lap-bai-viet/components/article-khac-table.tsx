...
const ArticleKhacTable = memo(function ArticleKhacTable({
  store,
  data,
  isLoading,
  onRowClick,
  ...
  const renderCell = useCallback(
    (colId: string, item: BaiVietThietLapKhac) => {
      switch (colId) {
        case 'thu_tu':
  ...
  const renderMobileCard = useCallback(
    (item: BaiVietThietLapKhac, isSelected: boolean) => (
      <div
        key={item.id}
        role="button"
        tabIndex={0}
        ...
        <div className="flex items-start gap-3 sm:gap-4">
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex items-center justify-between pt-2 border-t border-border">
              <span className="text-xs text-muted-foreground tabular-nums">{txt('page.articleSettings.colThuTu')}: {item.thu_tu}</span>
              <ArticleKhacRowActions
                compact
                item={item}
                menuOpenId={rowMenuOpenId}
...
export default ArticleKhacTable;

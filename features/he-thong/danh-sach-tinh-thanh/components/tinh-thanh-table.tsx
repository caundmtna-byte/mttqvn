...
import { TinhThanhRowActions } from './tinh-thanh-row-actions';
...
const TinhThanhTable = memo(function TinhThanhTable({
  ...
  const renderCell = useCallback(
    (colId: string, item: TinhThanh) => {
      switch (colId) {
        ...
        case 'so_xa_phuong':
          return (
            <span className="text-sm font-medium tabular-nums text-foreground">{item.so_xa_phuong ?? 0}</span>
          );
        case 'tg_tao':
  ...
  const renderMobileCard = useCallback(
    (item: TinhThanh, isSelected: boolean) => (
      <div
        key={item.id}
        role="button"
        tabIndex={0}
        ...
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-xs text-muted-foreground">
              ...
              {' · '}
              {txt('diaBan.store.soXaPhuongCol')}:{' '}
              <span className="tabular-nums font-medium text-foreground">{item.so_xa_phuong ?? 0}</span>
            </p>
          </div>
        </div>

...
import { countPbxhColumnSearchActive } from '../utils/column-search';

interface Props {
  store: GenericState<PbxhThietLapFilters>;
  items?: PbxhThietLap[] | null;
  tabSlot: ReactNode;
  onPageBack?: () => void;
  onAdd: () => void;
  onExport: () => void;
...
const PbxhThietLapToolbar: React.FC<Props> = ({
  store,
  items,
  tabSlot,
  onPageBack,
  ...
  const { canCreate, canExport, canDelete } = useResourcePermissions('phanBienThietLapDanhMuc');
  ...
  const moTaChipOptions = useMemo(
    () => [
      { label: txt('page.pbxhThietLap.filterMoTaHas'), value: 'has', count: moTaCounts.has },
      { label: txt('page.pbxhThietLap.filterMoTaEmpty'), value: 'empty', count: moTaCounts.empty },
    ],
    [moTaCounts.has, moTaCounts.empty],
  );

  const activeFilterCount = useMemo(() => {
    const colN = countPbxhColumnSearchActive(filters.columnSearch, filters.mo_ta_bucket);
    ...
    () => (
      <div className="flex flex-wrap items-center gap-2 min-w-0">
        <FilterChipSingleSelect
          ...
          placeholder={txt('page.pbxhThietLap.filterMoTaChipPlaceholder')}
          icon={AlignLeft}
      ...
      {canExport && (
        <Tooltip content={txt('common.export')} placement="bottom">
          <Button
            ...
            size="sm"
            onClick={onExport}
            className="h-8 w-8 p-0 flex items-center justify-center border-border text-muted-foreground hover:bg-muted/50"
          >
...
export default PbxhThietLapToolbar;

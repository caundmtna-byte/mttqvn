-- ============================================================================
-- get_nddk_page — phân trang phía máy chủ cho module "Nhà đại đoàn kết".
--
-- Bảng giao dịch, không có trần tự nhiên ⇒ không phân trang ở client.
-- RPC trả luôn cột hiển thị đã LEFT JOIN (tên xã/phường, tên người tạo) để mỗi
-- lần đổi trang chỉ tốn MỘT request.
--
-- Phạm vi xem khớp `canViewNddkRow` ở client: cấp Xã phường chỉ thấy dòng có
-- `xa_phuong_id` trùng đơn vị mình. Đây mới là lớp chặn thật cho trang danh sách.
-- ============================================================================
...
CREATE FUNCTION public.get_nddk_page(
...
AS $$
  ...
  WHERE (
      ...
      OR s.ho_ten_chu_ho   ILIKE '%' || p_search || '%'

-- Bổ sung lọc nguồn đăng / trang đăng (chip) cho get_bai_viet_page — tham số cuối DEFAULT NULL.

DROP FUNCTION IF EXISTS public.get_bai_viet_page(text, integer, integer, text, bigint, bigint, bigint[]);
...
COMMENT ON FUNCTION public.get_bai_viet_page(
  ...
  'Egress: pagination + search + scope + thể loại + nguồn đăng + trang đăng (server-side).';

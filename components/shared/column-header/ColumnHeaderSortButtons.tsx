...
/** Nút sắp xếp A→Z / Z→A trong panel header cột (dùng chung listview). */
    ...
    <div className="flex flex-col gap-0.5">
      <button
        ...
        className={cn(
          ...
          isAscActive ? 'bg-primary/15 font-medium text-primary' : 'text-foreground hover:bg-muted/80',
        )}
        onClick={() => {
      ...
      >
        ...
        <span>{txt('common.sortAsc')}</span>
      </button>
      <button
        type="button"
        className={cn(
          'flex w-full items-center gap-2 rounded-lg px-2 py-1.5 text-left text-xs transition-colors',
          isDescActive ? 'bg-primary/15 font-medium text-primary' : 'text-foreground hover:bg-muted/80',
        )}
        onClick={() => {
